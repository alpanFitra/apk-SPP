...
class Datakelas extends CI_Controller {
	...
	public function index(){
		...
		if($cek_user_level == 'admin'){
			$data['semua'] = $this->model_datakelas->all();
	...
	public function form(){
		...
		if($cek_user_level == 'admin'){
			...
			if ($this->form_validation->run() == TRUE){
				...
				//panggil method insert pada Model model_datakelas
				$this->model_datakelas->insert($array);
				...
				//panggil validation
				$this->load->view('kelas/datakelasform',$data);
			}
		}
		else{
			redirect('notfound');
		...
		if($cek_user_level == 'admin'){
			...
			if($this->input->post('ubah')){
				...
				if ($this->form_validation->run() == TRUE){
					...
					//panggil method insert pada Model model_datakelas
					$this->model_datakelas->update($array,array('id_kelas'=>$this->input->post('idkelas')));
					//kembali ke daftar tabel
					$url=base_url().'datakelas/';
					redirect($url);
				}
					...
					$this->load->view('kelas/datakelasformedit',$data);
				}
			}
			else{
				//id diambil dari URL SEGMENT 3
				$data['satu'] = $this->model_datakelas->getwhere(array('id_kelas'=>$this->uri->segment(3)))->row_array();
				$this->load->view('kelas/datakelasformedit',$data);
			}
		}
		else{
	...
	public function hapus(){
		...
		if($cek_user_level == 'admin'){
			...
			//dengan memanggil method model model_datakelas
			if($this->uri->segment(3)) $this->model_datakelas->delete(array('id_kelas'=>$this->uri->segment(3)));
			//kembali ke daftar tabel
			$url=base_url().'datakelas/';
			redirect($url);
		}
