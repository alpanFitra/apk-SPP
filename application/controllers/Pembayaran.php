...
class Pembayaran extends CI_Controller {
	...
	public function index(){
		...
		if($cek_user_level == 'admin' or $cek_user_level == 'petugas'){
			$data['siswa'] = $this->model_pembayaran->carisiswa();
		...
		if($cek_user_level == 'admin' or $cek_user_level == 'petugas'){
			if($this->input->post('proses')){
				...
				if ($this->form_validation->run() == TRUE){
					...
					$cekbayar = $this->model_pembayaran->cekbayar($nisn,$tahun,$bulan);
					if($cekbayar['result'] == "success"){
						...
						$this->model_pembayaran->insert($array);
						...
						$data['satu'] = $this->model_pembayaran->getwhere(array('nisn'=>$this->uri->segment(3)))->row_array();
						$data['databayar'] = $this->model_pembayaran->databayar($cnisn);
						$tgl_sekarang = date("Ymd");
						$this->load->view('pembayaran/pembayaranform',$data);
					}
				}
				else{
					//masukkan data POST ke dalam variabel $array
	...
	function kwitansi($slug){ //fungsi untuk menampilkan detail kwitansi
		$cek_user_level = $this->session->userdata('akses');
		if($cek_user_level == 'admin' or $cek_user_level == 'petugas'){
			$sql=$this->model_pembayaran->get_kwitansi_by_slug($slug);
			if($sql->num_rows() > 0){
					
				//tampil kwitansi
				$data['kwitansi']= $sql;
					
				$this->load->view('pembayaran/pembayarankwitansi',$data);
			}
			else{
